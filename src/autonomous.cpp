...
void ultimate_drive_straight(int max_linear_speed, int max_angular_speed, float target_heading, float target_distance,float acceptable_error, float max_time, float linear_kP, float linear_kI, float linear_kD, float angular_kP, float angular_kI, float angular_kD, float max_accel){
    float end_time = master_timer.time(seconds) + max_time;
    ...
    float initial_position = left_front_motor.position(degrees);
    float current_position = massive_distance_sensor.objectDistance(inches);
    ...
    while(true){
        ...
        left_drive_linear_velocity = -1*(linear_kP * linear_error + linear_kI * linear_integral_error + linear_kD * linear_derivative_error);
        right_drive_linear_velocity = -1*(linear_kP * linear_error + linear_kI * linear_integral_error + linear_kD * linear_derivative_error);
...
void color_sorting_intake(){
    ...
    double red_hue_difference = 1;
    double blue_hue_difference = 218;
    ...
    while (color_sorting_running){
        ...
        if(color_sorting_sensor.isNearObject()){
            ...
            std::cout << "block color: " <<block_color << " | blue diff: " << blue_hue_difference << " | red diff:" << red_hue_difference << std::endl;
